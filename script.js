document.addEventListener('DOMContentLoaded', () => {
    ...
    */
    ...
    const botonBorrarVentas = document.getElementById('boton-borrar-ventas');
    ...
    async function cargarEstadoDelJuego() {
        try {
            ...
            for (const response of responses) {
                if (!response.ok) {
                    throw new Error(`Una de las peticiones al servidor falló: ${response.status} ${response.statusText}`);
                }
            }
    ...
    function verificarGanadores() {
        ...
        cartonesActivos.forEach(carton => {
            ...
            let esGanador = false;
            const patrones = { 'cartonlleno': Array.from({ length: 25 }, (_, i) => i),'lnormal': [0, 5, 10, 15, 20, 21, 22, 23, 24],'4esquinas': [0, 4, 20, 24],'x': [0, 6, 12, 18, 24, 4, 8, 16, 20],'cruzgrande': [2, 7, 10, 11, 12, 13, 14, 17, 22],'bordecarton': [0, 1, 2, 3, 4, 5, 9, 10, 14, 15, 19, 20, 21, 22, 23, 24],'fila_1': [0, 1, 2, 3, 4],'fila_2': [5, 6, 7, 8, 9],'fila_3': [10, 11, 12, 13, 14],'fila_4': [15, 16, 17, 18, 19],'fila_5': [20, 21, 22, 23, 24],'columna_1': [0, 5, 10, 15, 20],'columna_2': [1, 6, 11, 16, 21],'columna_3': [2, 7, 12, 17, 22],'columna_4': [3, 8, 13, 18, 23],'columna_5': [4, 9, 14, 19, 24],'linvertida': [20, 21, 22, 23, 24, 4, 9, 14, 19],'cruzpequeña': [7, 11, 12, 13, 17],'e': [0, 5, 10, 15, 20, 1, 11, 21],'t': [0, 1, 2, 3, 4, 7, 12, 17, 22]};

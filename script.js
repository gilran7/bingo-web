...
async function guardarCartonesEnAlmacen() {
    ...
    try {
        ...
        // Esta es la nueva lógica que soluciona el problema
...
function generarMatrizDeCarton() {
    const numerosPorColumna = {
        B: { min: 1, max: 15, numeros: [] },
        I: { min: 16, max: 30, numeros: [] },
        N: { min: 31, max: 45, numeros: [] },
        G: { min: 46, max: 60, numeros: [] },
        O: { min: 61, max: 75, numeros: [] }
    };
    let matriz = Array(5).fill(null).map(() => Array(5));
    for (let i = 0; i < 5; i++) {
        const letra = Object.keys(numerosPorColumna)[i];
        for (let j = 0; j < 5; j++) {
            if (i === 2 && j === 2) {
                matriz[j][i] = 'FREE';
            } else {
                let numero;
                const columna = numerosPorColumna[letra];
                do {
                    numero = Math.floor(Math.random() * (columna.max - columna.min + 1)) + columna.min;
                } while (columna.numeros.includes(numero));
...
function cantarNumeroAutomatico() {
    ...
    } while (numerosCantados.includes(nuevoNumero));
...
function actualizarTodosDisplays() {
    ...
    document.querySelectorAll(".carton-individual td.marcado").forEach(c => {
        if (c.textContent !== "★") c.classList.remove("marcado")
    });
    ...
    numerosCantados.forEach(num => {
        ...
        cartonesEnJuego.forEach(carton => {
            for (let i = 0; i < 5; i++)
                for (let j = 0; j < 5; j++)
                    if (carton.matriz[i][j] === num) carton.elemento.querySelector("tbody").rows[i].cells[j].classList.add("marcado")
        })
    });
...
function verificarGanadores() {
    ...
    cartonesActivos.forEach(carton => {
        const celdas = Array.from(carton.elemento.querySelector("tbody").rows).map(row => Array.from(row.cells));
        let esGanador = false;
        const isMarked = (r, c) => celdas[r][c].classList.contains("marcado");
        switch (patron) {
            case "fila":
                for (let i = 0; i < 5; i++) {
                    if (celdas[i].every(c => c.classList.contains("marcado"))) esGanador = true
                }
                break;
            case "columna":
                for (let i = 0; i < 5; i++) {
                    if (celdas.every(f => f[i].classList.contains("marcado"))) esGanador = true
                }
                break;
            case "lnormal":
                esGanador = celdas.every(row => row[0].classList.contains("marcado")) && celdas[4].every(c => c.classList.contains("marcado"));
                break;
            case "linvertida":
                esGanador = celdas.every(row => row[4].classList.contains("marcado")) && celdas[0].every(c => c.classList.contains("marcado"));
                break;
            case "e":
                esGanador = celdas.every(row => row[0].classList.contains("marcado")) && celdas[0].every(c => c.classList.contains("marcado")) && celdas[2].every(c => c.classList.contains("marcado")) && celdas[4].every(c => c.classList.contains("marcado"));
                break;
            case "x":
                esGanador = celdas.every((row, i) => row[i].classList.contains("marcado")) && celdas.every((row, i) => row[4 - i].classList.contains("marcado"));
                break;
            case "4esquinas":
                esGanador = isMarked(0, 0) && isMarked(0, 4) && isMarked(4, 0) && isMarked(4, 4);
                break;
            case "cruzpequeña":
                esGanador = isMarked(1, 2) && isMarked(2, 1) && isMarked(2, 2) && isMarked(2, 3) && isMarked(3, 2);
                break;
            case "cruzgrande":
                esGanador = celdas[2].every(c => c.classList.contains("marcado")) && celdas.every(f => f[2].classList.contains("marcado"));
                break;
            case "t":
                esGanador = celdas[0].every(c => c.classList.contains("marcado")) && celdas.every(row => row[2].classList.contains("marcado"));
                break;
            case "bordecarton":
                esGanador = celdas[0].every(c => c.classList.contains("marcado")) && celdas[4].every(c => c.classList.contains("marcado")) && celdas.every(row => row[0].classList.contains("marcado")) && celdas.every(row => row[4].classList.contains("marcado"));
                break;
            case "cartonlleno":
                esGanador = celdas.flat().every(c => c.classList.contains("marcado"));
                break
        }
...
function verificarDuplicados() {
    ...
    for (let i = 0; i < matricesString.length; i++) {
        for (let j = i + 1; j < matricesString.length; j++) {
            if (matricesString[i] === matricesString[j]) {
                duplicados.push(`- Cartón #${i+1} y Cartón #${j+1}`)
            }
        }
    }
    if (duplicados.length > 0) {
        ...
        alert("Verificación completada. No se encontraron cartones repetidos.")
    }
}
...
botonBorrarCartones.addEventListener('click', () => {
    if (confirm('¿Estás seguro de que quieres borrar todos los cartones?')) {
        reiniciarSistemaCompleto();
    }
});
...
selectPatron.addEventListener('change', () => {
    ...
    imagenPatron.onerror = () => {
        imagenPatron.src = 'imagenes/patron_cartonvacio.png';
    };
    guardarEstadoDelJuego();
});
